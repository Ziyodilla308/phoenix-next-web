    ...
    <div className="lg:mt-25 lg:mb-25 mt-15 mb-15">
      ...
      <div>
        <Swiper
          ...
          spaceBetween={25} // slidelar orasida biroz bo'shliq
          breakpoints={{
            320: { slidesPerView: 1.5 },
            640: { slidesPerView: 2.5 },
            768: { slidesPerView: 3.5 },
            1024: { slidesPerView: 4.5 },
            1280: { slidesPerView: 5.3472 },
            1536: { slidesPerView: 5.3472 },
          }}
          grabCursor
          modules={[Autoplay]}
          ...
          {categories.map(({ categoryName, id, categoryImg }) => (
            <SwiperSlide key={id}>
              <div className="text-center px-2">
                {" "}
                {/* cardga padding qo'shildi */}
                {/* Rasm container - aspect ratio 1:1, kattaroq height */}
                <Link
                  href={`/category?name=${categoryName}`}
                  className="block w-full max-w-[1000px] mx-auto mb-5 rounded-[20px] overflow-hidden aspect-[1/1] relative"
                >
                  <Image
                    src={categoryImg}
                    alt={categoryName}
                    fill
                    className="object-cover w-full h-full hover:scale-105 transition-all duration-500 rounded-[20px]"
                  />
                </Link>
                {/* Category Name */}
                <Link
